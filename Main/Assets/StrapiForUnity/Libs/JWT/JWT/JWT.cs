...
namespace JWT
{
    ...
    public static class JsonWebToken
    {
        ...
        public static string Decode(string token, byte[] key, bool verify = true)
        {
            ...
            var headerData =  JsonMapper.ToObject(headerJson);
        ...
        /// <exception cref="SignatureVerificationException">Thrown if the verify parameter was true and the signature was NOT valid or if the JWT was signed with an unsupported algorithm.</exception>
        public static JsonData DecodeToObject(string token, byte[] key, bool verify = true)
        {
            var payloadJson = Decode(token, key, verify);
            JsonData payloadData = JsonMapper.ToObject(payloadJson);
            return payloadData;
        }
        ...
        /// <returns>An object representing the payload.</returns>
        /// <exception cref="SignatureVerificationException">Thrown if the verify parameter was true and the signature was NOT valid or if the JWT was signed with an unsupported algorithm.</exception>
        public static JsonData DecodeToObject(string token, string key, bool verify = true)
        {
